...
mod tests {
  ...
  fn test1() {
    ...
    assert_eq!(solve(number_list), 175594);
  }

  #[test]
  fn test2() {
    let number_list: Vec<i32> = vec![1,3,2];
    assert_eq!(solve(number_list), 2578);
  }

  #[test]
  fn test3() {
    let number_list: Vec<i32> = vec![2,1,3];
    assert_eq!(solve(number_list), 3544142);
  }

  #[test]
  fn test4() {
    let number_list: Vec<i32> = vec![1,2,3];
    assert_eq!(solve(number_list), 261214);
  }

  #[test]
  fn test5() {
    let number_list: Vec<i32> = vec![2,3,1];
    assert_eq!(solve(number_list), 6895259);
  }

  #[test]
  fn test6() {
    let number_list: Vec<i32> = vec![3,2,1];
    assert_eq!(solve(number_list), 18);
  }

  #[test]
  fn test7() {
    let number_list: Vec<i32> = vec![3,1,2];
    assert_eq!(solve(number_list), 362);
  }
}
